...
public class DishServiceImpl implements DishService {
    ...
    public void addDish(Dish dish) {
        ...
        if (base64Image != null && !base64Image.isEmpty()) {
            fileName = UUID.randomUUID().toString() + ".jpg"; // 一意のファイル名を生成
        ...
        if (fileName != null) {
            ...
            saveFile.SaveImage(base64Image, dishImagePath, 400); // 大サイズ画像を保存
            saveFile.SaveImage(base64Image, orderImagePath, 200); // 小サイズ画像を保存
        }
    }
    ...
    public void updateDishImage(Dish dish) {
        // 変数の初期化
        String base64Image = dish.getImage(); // Base64形式の画像データを取得
        String fileName = dish.getName(); // 画像ファイル名
        String dishImagePath = null; // 大サイズ画像の保存パス
        String orderImagePath = null; // 小サイズ画像の保存パス
